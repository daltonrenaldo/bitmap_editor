...
describe BitmapEditor::CommandRunner do
  ...
  describe '#execute' do
    context 'bitmap responds to line command' do
      before do
        allow(bitmap).to receive(:respond_to?).with(:create_bitmap).and_return(true)
      end

      it 'perfoms the command' do
        expect(bitmap).to receive(:create_bitmap).with(3, 4)
        subject.execute("I 3 4", bitmap)
      end
    end
    ...
    context 'bitmap does not respond to line command' do
      ...
      it 'outputs unrecognised command' do
        ...
        subject.execute("J 2 1 4", bitmap)
      end
    end

    context 'line has invalid format' do
      it 'does nothing' do
        expect(subject.execute("04", bitmap)).to eql nil
      end
    end
